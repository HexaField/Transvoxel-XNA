import { Tables } from '../lengyel/tables'
import {
  createMatrix3x3,
...
const prevOffset = (dir: number): Vector3i => createVector3i(-(dir & 1), -((dir >> 1) & 1), -((dir >> 2) & 1))
...
const createVertex = (
  ...
  if (near > 0) {
    ...
    const projected = multiplyVector3fScalar(
      projectNormal(normal, delta, vertexProjectionScratch),
      cellSize,
      vertexProjectionScratch
    )
    return {
      primary,
...
const crossVector3i = (a: Vector3i, b: Vector3i): Vector3i =>
  createVector3i(a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x)
...
export const TransvoxelExtractor = {
    ...
    for (let i = 0; i < vertexCount; i++) {
      ...
      if ((t & 0x00ff) !== 0) {
        ...
        if (present) {
          ...
          if (prev.caseIndex === 0 || prev.caseIndex === 255 || prev.dirs[idx] !== axisBits) {
        ...
        if (present) {
          const prev = cache.getCellByVector(addVector3i(xyz, prevOffset(dir)))
          if (prev.caseIndex === 0 || prev.caseIndex === 255 || prev.dirs[0] !== axisBits) {
            localVertexMapping[i] = -1
          } else {
        ...
        if (!present || localVertexMapping[i] < 0) {
          const pi = subtractVector3f(combineVector3f(toVector3f(p0), t0, toVector3f(p1), t1), blockOriginF)
          const normal = computeNormal(n0, n1, t0, t1)
          const vertex = createVertex(pi, normal, near, offset, lodIndex, cellSize)
          localVertexMapping[i] = verts.push(vertex) - 1
        }
  ...
  polygonizeTransitionCell(
    ...
    for (let i = 0; i < transitionCoordinates.length; i++) {
      ...
      setVector3i(positions[i], origin.x + transformed.x, origin.y + transformed.y, origin.z + transformed.z)
    }
