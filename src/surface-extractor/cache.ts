...
class RegularCacheCell implements RegularCacheCellView {
  ...
  constructor(
    caseIndexArray: Int16Array,
    vertsArray: Int32Array,
    dirsArray: Int8Array,
    cellIndex: number
  ) {
  ...
  constructor(private readonly blockWidth: number) {
    ...
    this.cells = Array.from({ length: blockWidth }, (_, x) =>
      Array.from({ length: blockWidth }, (_, y) =>
        Array.from({ length: blockWidth }, (_, z) =>
          new RegularCacheCell(this.caseIndex, this.verts, this.dirs, this.indexOf(x, y, z))
        )
      )
    );
  }
...
export class TransitionCache {
  ...
  constructor(private readonly blockWidth: number) {
    ...
    this.cells = Array.from({ length: blockWidth }, (_, x) =>
      Array.from({ length: blockWidth }, (_, y) =>
        new TransitionCacheCell(this.caseIndex, this.verts, this.indexOf(x, y))
      )
    );
  }
