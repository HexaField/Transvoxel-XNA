...
const regularCaseDefinitions = Array.from(
  { length: 256 },
  (_, code) => createRegularCaseDefinition(code)
);
const transitionCaseDefinitions = Array.from(
  { length: 512 },
  (_, code) => createTransitionCaseDefinition(code)
);
...
function createCaseCard(def: CaseDefinition): HTMLElement {
  ...
  statsList.appendChild(
    createStatBlock(
      "Actual",
      `${stats.actualVertices} verts · ${stats.actualTriangles} tris`
    )
  );
...
function getCaseStats(def: CaseDefinition): CaseStats {
  ...
  const meshData = def.kind === "regular"
    ? buildRegularCaseMesh(def.caseCode)
    : buildTransitionCaseMesh(def.caseCode);

  const actualVertices = meshData.vertices.length;
  const actualTriangles = meshData.indices.length / 3;
  const matchesExpected =
    actualVertices === def.expectedVertices &&
    actualTriangles === def.expectedTriangles;
...
class FacePreview {
  ...
  constructor(container: HTMLElement, meshData: MeshData, color: number) {
    ...
    const { mesh, radius, normalHelper, invertedHelper } = this.createScaledMesh(
      meshData,
      color
    );
  ...
  private createScaledMesh(
    ...
    const invertedHelper = orientation.invertedCount > 0
...
function createTransitionCaseDefinition(caseCode: number): CaseDefinition {
  ...
  return {
    ...
    expectedVertices: entry.getVertexCount(),
    expectedTriangles: entry.getTriangleCount(),
    summary: `${filled} / 9 high-res samples solid (mask ${binaryMask}). ` +
...
async function copyCaseSummary(
  ...
  const summary = `${def.title} → ${def.summary} Expected ${def.expectedVertices}/${def.expectedTriangles} vs ` +
...
function handlePreviewIntersection(entries: IntersectionObserverEntry[]): void {
  for (const entry of entries) {
    ...
    const stats = getCaseStats(def);
    const previewInstance = new FacePreview(
      container,
      stats.meshData,
      kind === "regular" ? 0x5bc7ff : 0xffb347
    );
