export type TransitionFace = 'negativeX' | 'positiveX' | 'negativeY' | 'positiveY' | 'negativeZ' | 'positiveZ'
...
export class OctreeChunkManager {
  ...
  constructor(private readonly config: OctreeChunkConfig) {
    ...
    this.derivedLodLevels = config.lodLevels
      .map((level) => ({
        ...level,
        maxDistance: level.maxDistance ?? this.chunkWorldSize(level.lodIndex) * this.lodDistanceMultiplier
      }))
  ...
  update(cameraPosition: Vector3Like): ChunkPlan {
    ...
    const teleported = previousPosition
      ? this.distanceBetween(previousPosition, cameraPosition) > this.teleportThreshold()
      : false
  ...
  private reconcile(desired: Map<string, ChunkDescriptor>, teleported: boolean): ChunkPlan {
    const plan = this.createPlan()
    ...
    for (const [key, record] of this.activeChunks.entries()) {
      ...
      if (desiredDescriptor) {
        if (this.transitionFacesChanged(record.transitionFaces, desiredDescriptor.transitionFaces)) {
  ...
  private collectDesiredChunks(cameraPosition: Vector3Like): Map<string, ChunkDescriptor> {
    ...
    while (queue.length) {
      ...
      const culled = !node.forceInclude && node.lodIndex !== this.maxLodIndex && distance > level.maxDistance + radius
      if (culled) {
        continue
      }

      if (this.shouldSubdivide(node, distance)) {
        this.subdivideNode(node).forEach((child) => queue.push({ ...child, forceInclude: true }))
        continue
      }

      const key = this.chunkKey(node.lodIndex, node.chunkX, node.chunkY, node.chunkZ)
      if (!desired.has(key)) {
  ...
  private pruneDistantChunks(
    ...
    const travelDistance = Math.hypot(travelVector.x, travelVector.y, travelVector.z)
    if (travelDistance === 0) {
      return
    }

    const coarseSize = this.chunkWorldSize(this.maxLodIndex)
    const farthestLevel = this.derivedLodLevels[this.derivedLodLevels.length - 1]
    const movementThreshold = Math.max(coarseSize * 2, farthestLevel.maxDistance * 0.5)
    ...
    for (const descriptor of this.activeChunks.values()) {
      if (descriptor.lodIndex === this.maxLodIndex && !desired.has(descriptor.key)) {
        coarseDescriptors.push(descriptor)
      }
    }
  ...
  private hasActiveDescendants(descriptor: ChunkDescriptor): boolean {
    for (const record of this.activeChunks.values()) {
      if (record.lodIndex < descriptor.lodIndex && this.isDescendantChunk(descriptor, record)) {
        return true
      }
    }
    for (const pending of this.pendingRequests.values()) {
      const child = pending.descriptor
      if (child.lodIndex < descriptor.lodIndex && this.isDescendantChunk(descriptor, child)) {
        return true
      }
    }
    return false
  ...
  private removeDescendantDesiredChunks(desired: Map<string, ChunkDescriptor>, parent: ChunkDescriptor): void {
  ...
  private isDescendantChunk(parent: ChunkDescriptor, child: ChunkDescriptor): boolean {
  ...
  private generateSpiralOrder(
    ...
    const coords: Array<{ chunkX: number; chunkZ: number }> = [{ chunkX: centerX, chunkZ: centerZ }]
    ...
    while (coords.length < totalCells) {
      for (let repeat = 0; repeat < 2; repeat++) {
        ...
        for (let step = 0; step < stepLength && coords.length < totalCells; step++) {
          x += dx
          z += dz
  ...
  private chunkDistance(node: OctreeNode, point: Vector3Like): number {
    return this.chunkDistanceToPoint(node.lodIndex, node.chunkX, node.chunkY, node.chunkZ, point)
  }
  ...
  private descriptorPriority(descriptor: ChunkDescriptor): number {
    return Math.abs(descriptor.chunkX) + Math.abs(descriptor.chunkY) + Math.abs(descriptor.chunkZ)
  }
  ...
  private scheduleSplit(parent: ChunkDescriptor, childDescriptors: ChunkDescriptor[], plan: ChunkPlan): void {
    console.debug(
    ...
    childDescriptors.forEach((descriptor) => {
      ...
      if (!this.activeChunks.has(descriptor.key) && !this.emptyChunks.has(descriptor.key)) {
  ...
  private scheduleMerge(parentDescriptor: ChunkDescriptor, plan: ChunkPlan): void {
    this.ensureChunkRequested(parentDescriptor, plan)
    const parentKey = parentDescriptor.key
    const childKeys = this.computeChildKeys(parentDescriptor).filter((childKey) => this.activeChunks.has(childKey))
  ...
  private resolveParentRetention(descriptor: ChunkDescriptor, plan: ChunkPlan): void {
    const parentKey = this.getParentKey(descriptor.lodIndex, descriptor.chunkX, descriptor.chunkY, descriptor.chunkZ)
    if (!parentKey) {
      return
    }
  ...
  private completePendingMerge(parentKey: string, plan: ChunkPlan): void {
    ...
    console.debug('[OctreeChunkManager] merge complete', parentKey, Array.from(children))
    children.forEach((childKey) => this.disposeChunk(childKey, plan))
    this.pendingMergeParents.delete(parentKey)
  }

  private ensureChunkRequested(descriptor: ChunkDescriptor, plan: ChunkPlan): void {
    if (
      this.activeChunks.has(descriptor.key) ||
      this.pendingRequests.has(descriptor.key) ||
      this.emptyChunks.has(descriptor.key)
  ...
  private collectChildDescriptors(record: ChunkDescriptor, desired: Map<string, ChunkDescriptor>): ChunkDescriptor[] {
  ...
  private findParentDescriptor(record: ChunkDescriptor, desired: Map<string, ChunkDescriptor>): ChunkDescriptor | null {
    if (record.lodIndex >= this.maxLodIndex) {
      return null
    }
    const parentKey = this.getParentKey(record.lodIndex, record.chunkX, record.chunkY, record.chunkZ)
    if (!parentKey) {
      return null
    }
  ...
  private getParentKey(lodIndex: number, chunkX: number, chunkY: number, chunkZ: number): string | null {
  ...
  private assignTransitionFaces(desired: Map<string, ChunkDescriptor>): void {
    ...
    desired.forEach((descriptor) => {
      descriptor.transitionFaces = this.computeTransitionFaces(descriptor, boundsByKey, levelBuckets)
    })
  }
  ...
  private evaluateTransitionAdjacency(coarse: ChunkBounds, neighbor: ChunkBounds, faces: Set<TransitionFace>): void {
    if (
    ...
    if (
      ...
      this.rangesOverlap(neighbor.minX, neighbor.maxX, coarse.minX, coarse.maxX) &&
      this.rangesOverlap(neighbor.minZ, neighbor.maxZ, coarse.minZ, coarse.maxZ)
    ) {
      faces.add('negativeY')
    }
  ...
  private transitionFacesChanged(current: TransitionFace[], next: TransitionFace[]): boolean {
  ...
  private chunkKey(lodIndex: number, chunkX: number, chunkY: number, chunkZ: number): string {
